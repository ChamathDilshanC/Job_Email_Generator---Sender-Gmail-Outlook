...
const templates: { id: CoverLetterTemplate; name: string; description: string; className: string }[] = [
  ...
  { id: 'executive', name: 'Executive Navy', description: 'Premium navy header for senior applications', className: 'border-indigo-600 bg-indigo-50' },
  { id: 'modern', name: 'Modern Teal', description: 'Fresh teal details with clear job-focused hierarchy', className: 'border-teal-500 bg-teal-50' },
  { id: 'classic', name: 'Classic Serif', description: 'Traditional recruiter-friendly format with warm accents', className: 'border-amber-600 bg-amber-50' },
];
...
export default function CoverLetter() {
  ...
  const save = async () => {
    ...
    setIsBusy(true);
    try {
      const response = await fetch('/api/cover-letter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.uid, profileId, profileName: selectedProfile?.profileName, name: letterName, ...form, content, id: currentId }) });
      const data = await response.json();
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        await fetch('/api/cover-letter', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.uid, profileId, profileName: selectedProfile?.profileName, name: letterName, ...form, content, id: currentId }) });
      } catch (error) {
  ...
  const openLetter = (letter: CoverLetter) => { setCurrentId(letter.id); setProfileId(letter.profileId); setContent(letter.content); setLetterName(letter.name || `${letter.companyName} - ${letter.position}`); setForm({ ...emptyLetter, ...letter }); };
