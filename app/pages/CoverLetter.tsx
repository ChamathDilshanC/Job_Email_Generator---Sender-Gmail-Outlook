...
import { type CoverLetter, type CoverLetterLength, type CoverLetterTone } from '@/lib/coverLetter';
import { listResumeProfiles, loadResumeData, type ResumeData, type ResumeProfileSummary } from '@/lib/resumeDataService';
import { showToast } from '@/lib/toast';
import { Copy, Download, FileText, Loader2, RefreshCw, Save, Sparkles, Trash2 } from 'lucide-react';
...
const emptyLetter = { companyName: '', position: '', jobUrl: '', jobDescription: '', length: 'standard' as CoverLetterLength, tone: 'professional' as CoverLetterTone, additionalInstructions: '', hiringManagerName: '', hiringManagerTitle: '', companyAddress: '', includeContactHeader: true };
...
export default function CoverLetter() {
  ...
  const importJob = async () => {
    ...
    try {
      ...
      setForm(prev => ({ ...prev, jobUrl, companyName: data.companyName || prev.companyName, position: data.position || prev.position }));
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(320px,0.8fr)_minmax(0,1.2fr)]">
      <Card><CardHeader><CardTitle>Job details</CardTitle><CardDescription>Choose a profile, import a posting, or enter details manually.</CardDescription></CardHeader><CardContent className="space-y-4">
        ...
        <div><Label htmlFor="description">Job Description *</Label><Textarea id="description" value={form.jobDescription} onChange={e => update('jobDescription', e.target.value)} maxLength={30000} rows={8} placeholder="Paste the job description here..." /><p className="mt-1 text-right text-xs text-muted-foreground">{form.jobDescription.length.toLocaleString()} / 30,000</p></div>
        <div><Label>Length</Label><div className="grid grid-cols-3 gap-1 rounded-lg border p-1">{lengths.map(value => <Button key={value} type="button" variant={form.length === value ? 'default' : 'ghost'} size="sm" onClick={() => update('length', value)}>{value[0].toUpperCase() + value.slice(1)}</Button>)}</div></div>
