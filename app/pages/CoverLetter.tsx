...
const templates: { id: CoverLetterTemplate; name: string; description: string; className: string }[] = [
  { id: 'minimal', name: 'ATS Minimal', description: 'Simple single-column layout for reliable parsing', className: 'border-slate-400 bg-slate-50' },
  { id: 'corporate', name: 'ATS Structured', description: 'Clear hierarchy with conventional recruiter spacing', className: 'border-blue-500 bg-blue-50' },
  { id: 'editorial', name: 'ATS Modern', description: 'Modern typography without columns, graphics, or tables', className: 'border-indigo-500 bg-indigo-50' },
];
...
export default function CoverLetter() {
  ...
  const previewContent = content || `I am excited to apply for the ${form.position || 'Target Position'} role at ${form.companyName || 'Target Company'}. My background and experience are aligned with the responsibilities described for this opportunity.\n\nMy experience has given me practical skills that can contribute to this role. I would welcome the opportunity to discuss how my background, relevant projects, and approach to delivering quality work could support your team.\n\nThank you for considering my application. I look forward to the opportunity to discuss the position further.`;
  const previewIsPlaceholder = !content;
  const actionButtonClass = 'h-9 rounded-lg border-slate-200 bg-white px-3 text-xs font-medium shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/50 hover:bg-primary/5 hover:text-primary';
  ...
  return <div className="w-full max-w-none space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(320px,0.8fr)_minmax(0,1.2fr)]">
      ...
      <Card className="bg-muted/30"><CardHeader className="flex-row items-center justify-between gap-3"><div><CardTitle>Preview</CardTitle><CardDescription>{content ? 'ATS-friendly single-column application version' : 'Live template preview · generate to replace the sample copy'}</CardDescription></div><div className="flex flex-wrap justify-end gap-2">{content && <><Button size="sm" variant="outline" className={actionButtonClass} onClick={() => setIsEditing(!isEditing)}><FileText /> {isEditing ? 'Preview' : 'Edit'}</Button><Button size="sm" variant="outline" className={actionButtonClass} onClick={() => navigator.clipboard.writeText(content).then(() => showToast('success', 'Cover letter copied to clipboard')).catch(() => showToast('error', 'Clipboard access failed'))}><Copy /> Copy</Button><Button size="sm" variant="outline" className={actionButtonClass} onClick={() => save()} disabled={isBusy}><Save /> Save</Button><Button size="sm" variant="outline" className={actionButtonClass} onClick={() => { try { exportCoverLetterPdf({ ...form, id: currentId || '', profileId, content, createdAt: '', updatedAt: '' }, resume); } catch (error) { showToast('error', 'PDF export failed', error instanceof Error ? error.message : undefined); } }}><Download /> PDF</Button><Button size="sm" variant="outline" className={actionButtonClass} onClick={() => exportCoverLetterDocx({ ...form, id: currentId || '', profileId, content, createdAt: '', updatedAt: '' }, resume).catch(() => showToast('error', 'DOCX export failed'))}><Download /> DOCX</Button><Button size="sm" variant="outline" className={actionButtonClass} onClick={regenerate}><RefreshCw /> Regenerate</Button></>}</div></CardHeader><CardContent><div className={`mx-auto h-[780px] max-h-[780px] max-w-[900px] overflow-hidden border border-slate-200 bg-white p-8 text-slate-900 shadow-sm sm:p-12 ${form.template === 'corporate' ? 'border-t-4 border-t-blue-700' : form.template === 'editorial' ? 'border-t-4 border-t-indigo-600' : 'border-t-4 border-t-slate-500'}`}>{isEditing ? <Textarea className="h-full min-h-[680px] resize-none border-0 p-0 text-[14px] leading-6 shadow-none focus-visible:ring-0" value={content} onChange={e => setContent(e.target.value)} /> : <div className={`text-[13px] leading-5 ${previewIsPlaceholder ? 'text-slate-400' : 'text-slate-900'}`}><div className="mb-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-400">Application Version · ATS-friendly</div>{form.includeContactHeader && <><h1 className="text-xl font-semibold text-slate-900">{name}</h1><p className="text-sm text-slate-500">{[resume?.personalInfo.email, resume?.personalInfo.phone, resume?.personalInfo.location].filter(Boolean).join(' · ') || 'email · phone · location'}</p><p className="mt-1 text-xs font-semibold text-slate-700">{form.position || 'Target Position'}</p></>}<p className="my-5 text-sm">{new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p><p className="text-sm">{form.hiringManagerName || 'Hiring Manager'}<br />{form.hiringManagerTitle || 'Hiring Manager'}<br />{form.companyName || 'Target Company'}<br />{form.companyAddress || 'Company address'}</p><p className="mt-4 text-sm">Dear {form.hiringManagerName || 'Hiring Manager'},</p>{previewContent.split(/\n\s*\n/).map((paragraph, index) => <p key={index} className="mt-3 whitespace-pre-wrap text-justify">{paragraph}</p>)}<p className="mt-5 text-sm">Sincerely,<br />{name}</p></div>}</div></CardContent></Card>
    </div>
